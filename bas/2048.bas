...
#context-free check Winner or Game Over
   When ( Summatory ( var(table) Is Gt (0)) Is Eq? ( var(SIZE) Mulby(SIZE) ) ) {
...
//   Let( sizet := var(SIZE) Minus ( Length( Compact(tmpTab), Copy to (ht) ) ) )
   var (SIZE),
   {tmpTab} Move to 'x1'
   Locate(1,60),  {","}toksep,Puts(x1), Puts("             \n") , {""}toksep
   Compact(tmpTab), Dup It, {","}toksep,Locate (1,80), puts, {""}toksep,Puts("                         ")
   Copy to (ht), 
   get Length It, Copy to 'l1'
      Locate (6,60), Puts("                             "),
      Locate (7,60), Puts("                             "),
      Locate (8,60), Puts("                             "),
      Locate (9,60), Puts("                             "),
      Locate (4,60), Puts("                             "),
      Locate(2,60),  {","}toksep,Puts(ht), Puts("             \n") , {""}toksep
      Locate(3,60), Puts(l1), Puts("             \n")
      Locate(20,1) 
   Sub, 
   Move to 'sizet'
...
Moving Left Up(tmpTab)
   ...
   var(SIZE), 
      {tmpTab} Move to 'x1'
   Locate(6,60),  {","}toksep,Puts(x1), Puts("             \n") , {""}toksep
   Compact( tmpTab ),
   Clear(tmpTab) 
   and Copy to (tmpTab); 
   get Length It, Copy to 'l1'
         ...
         Locate (4,60), Puts("                             "),
         Locate (9,60), Puts("                             "),
         Locate(7,60), {","}toksep,Puts(tmpTab), Puts("             \n") , {""}toksep
         Locate(8,60), Puts(l1), Puts("             \n")
         Locate(20,1)
   Sub; 
   then Move to (sizet)
   //Let( sizet := var(SIZE) Minus ( Length( Compact(tmpTab), Copy to (tmpTab) ) ) )
...
check Move Left Up(_DIRECTION_)
  ...
  For(i=1, while var(i) Is Le (SIZE),++i)
    If( _DIRECTION_ )
       Set Interval [i,1:SIZE]
       Locate (11,60), Print ("Moving Left  \n")
    Else 
       Set Interval [1:SIZE,i]
       Locate (11,60), Print ("Moving Up    \n")
    End If
